...
class TestM382CompressionCommitWarning:
    """Test M382: Warn on compression commits without validation."""

    def test_compression_commit_warns(self, handler):
        """Compression commit with skill files should warn about validation."""
        command = '''git commit -m "$(cat <<'EOF'
config: compress status skill documentation
EOF
)"'''
        with patch.object(handler, '_get_staged_files', return_value=['plugin/skills/status/SKILL.md']):
            result = handler.check(command, {})
        assert result is not None, "Compression commit should return warning"
        assert "additionalContext" in result, "Should add warning context, not block"
        assert "M382" in result["additionalContext"]
        assert "1.0" in result["additionalContext"]

    def test_shrink_commit_warns(self, handler):
        """Shrink commit with md files should warn about validation."""
        command = '''git commit -m "$(cat <<'EOF'
config: shrink documentation files
EOF
)"'''
        with patch.object(handler, '_get_staged_files', return_value=['docs/agent-architecture.md']):
            result = handler.check(command, {})
        assert result is not None, "Shrink commit should return warning"
        assert "additionalContext" in result

    def test_token_reduction_commit_warns(self, handler):
        """Token reduction commit should warn about validation."""
        command = '''git commit -m "$(cat <<'EOF'
config: token reduction for skills batch 4
EOF
)"'''
        with patch.object(handler, '_get_staged_files', return_value=['plugin/skills/work/SKILL.md']):
            result = handler.check(command, {})
        assert result is not None, "Token reduction commit should return warning"
        assert "additionalContext" in result

    def test_non_compression_commit_no_warning(self, handler):
        """Regular config commit should not warn."""
        command = '''git commit -m "$(cat <<'EOF'
...
EOF
        ...
        with patch.object(handler, '_get_staged_files', return_value=['plugin/skills/work/SKILL.md']):
            result = handler.check(command, {})
        assert result is None, "Non-compression commit should not warn"

    def test_compression_commit_non_doc_files_no_warning(self, handler):
        """Compression commit without doc files should not warn."""
        command = '''git commit -m "$(cat <<'EOF'
config: compress validation logic
EOF
)"'''
        with patch.object(handler, '_get_staged_files', return_value=['src/main.py']):
            result = handler.check(command, {})
        assert result is None, "Compression without doc files should not warn"
