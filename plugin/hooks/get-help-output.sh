#!/bin/bash
# get-help-output.sh - Pre-compute help display before skill runs
#
# TRIGGER: UserPromptSubmit
#
# When user invokes /cat:help, this hook builds the help content
# and returns it via additionalContext. The skill just outputs it.
...
# Check if this is a /cat:help command
if [[ ! "$USER_PROMPT" =~ ^[[:space:]]*/cat:help[[:space:]]*$ ]] && \
   [[ ! "$USER_PROMPT" =~ ^[[:space:]]*cat:help[[:space:]]*$ ]]; then
    echo '{}'
    exit 0
fi

# Build the workflow diagram using ASCII characters
# Original used Unicode arrows, now using ASCII for consistent width
WORKFLOW_DIAGRAM=$(cat << 'DIAGRAM'
```
...
# Build hierarchy tree using ASCII characters
HIERARCHY_TREE=$(cat << 'TREE'
```
.claude/cat/
...
# Build the complete help output
OUTPUT=$(cat << EOF
PRE-COMPUTED HELP DISPLAY (output exactly):
