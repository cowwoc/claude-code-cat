"""
Handler for /cat:work-with-issue precomputation.

Generates progress banners for all 4 work phases by parsing issue_id from skill args
...
class WorkWithIssueHandler:
    """Handler for /cat:work-with-issue skill."""

    def handle(self, context: dict) -> str | None:
        """
        Generate progress banners for all 4 work phases.

        Parses issue_id from the skill invocation JSON args and calls
        ...
        # Extract JSON args from prompt: /cat:work-with-issue {"issue_id": "..."}
        # The skill invocation can be:
        # - /cat:work-with-issue {"issue_id": "2.1-xxx", ...}
        # - cat:work-with-issue {"issue_id": "2.1-xxx"}
        match = re.search(r'/?\s*cat:work-with-issue\s+(\{[^}]*\})', user_prompt, re.IGNORECASE)
        if not match:
            return None
        ...
        try:
            ...
            result = subprocess.run(
                [str(banner_script), issue_id, "--all-phases"],
                capture_output=True,
                text=True,
                timeout=30
            )
            ...
            for phase in ["preparing", "executing", "reviewing", "merging"]:
                phase_result = subprocess.run(
                    [str(banner_script), issue_id, "--phase", phase],
                    capture_output=True,
                    text=True,
                    timeout=30
                )
...
# Register handler
_handler = WorkWithIssueHandler()
register_handler("work-with-issue", _handler)
