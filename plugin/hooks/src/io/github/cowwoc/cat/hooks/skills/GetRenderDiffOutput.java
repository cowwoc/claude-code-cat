...
public final class GetRenderDiffOutput
{
   ...
   * Parsed diff data.
   */
  private static class ParsedDiff
  {
   ...
   */
  private static class UsedSymbols
  {
    boolean minus = false;
    boolean plus = false;
    boolean space = false;
  ...
  private static class DiffParser
  {
      ...
      while (i < lines.length)
      {
        ...
        if (currentCommit != null && !inCommitMessage && !inDiffContent)
        {
          if (line.startsWith("Author:") || line.startsWith("Date:") ||
              line.startsWith("Merge:") || line.trim().isEmpty())
          {
            if (line.trim().isEmpty() && commitMessageLines.isEmpty())
    ...
    private void finalizeCommitMessage(Commit commit, List<String> lines)
    {
      ...
      if (lines.size() > 1)
      {
        ...
        while (!bodyLines.isEmpty() && bodyLines.get(0).trim().isEmpty())
          bodyLines.remove(0);
  ...
  private static class DiffRenderer
  {
    ...
    // Column widths
    private static final int COL_OLD = 4;
    private static final int COL_SYM = 3;
    ...
    private void printColumnHeader(String context)
    {
      ...
      output.add(
        BOX_T_RIGHT + fillChar(BOX_HORIZONTAL, COL_OLD) + BOX_T_DOWN +
        fillChar(BOX_HORIZONTAL, COL_SYM) + BOX_T_DOWN +
        fillChar(BOX_HORIZONTAL, COL_NEW) + BOX_T_DOWN +
        fillChar(BOX_HORIZONTAL, contentWidth + 1) + BOX_T_LEFT
      );

      // Header row with context
      String contextText = !context.isEmpty() ? "\u2381 " + context : "";
      int ctxLen = display.displayWidth(contextText);
      if (ctxLen > contentWidth)
        contextText = truncateToWidth(contextText, contentWidth - 1) + "\u2026";

      output.add(
        BOX_VERTICAL + padNum("Old", COL_OLD) + BOX_VERTICAL + "   " +
        BOX_VERTICAL + padNum("New", COL_NEW) + BOX_VERTICAL + " " +
        padRight(contextText, contentWidth) + BOX_VERTICAL
      );

      // Separator after headers
      output.add(
        BOX_T_RIGHT + fillChar(BOX_HORIZONTAL, COL_OLD) + BOX_CROSS +
        fillChar(BOX_HORIZONTAL, COL_SYM) + BOX_CROSS +
        fillChar(BOX_HORIZONTAL, COL_NEW) + BOX_CROSS +
        fillChar(BOX_HORIZONTAL, contentWidth + 1) + BOX_T_LEFT
      );
    }
    ...
    private void printRow(String oldNum, String symbol, String newNum, String content)
    {
      ...
      if (contentLen <= contentWidth)
      {
        ...
        output.add(
          BOX_VERTICAL + padNum(oldNum, COL_OLD) + BOX_VERTICAL + " " + symbol + " " +
          BOX_VERTICAL + padNum(newNum, COL_NEW) + BOX_VERTICAL + " " +
      ...
      else
      {
        ...
        String remaining = content.length() > contentWidth - 1 ? content.substring(contentWidth - 1) : "";
        while (!remaining.isEmpty())
        {
          ...
          if (partLen <= contentWidth)
          {
            ...
            output.add(
              BOX_VERTICAL + " ".repeat(COL_OLD) + BOX_VERTICAL + "   " +
              BOX_VERTICAL + " ".repeat(COL_NEW) + BOX_VERTICAL + " " +
              padded + BOX_VERTICAL
            );
            remaining = "";
          }
          else
          {
            ...
            remaining = remaining.length() > contentWidth - 1 ? remaining.substring(contentWidth - 1) : "";
          }
        }
      }
    }
    ...
    {
      output.add(
        BOX_BOTTOM_LEFT + fillChar(BOX_HORIZONTAL, COL_OLD) + BOX_T_UP +
        fillChar(BOX_HORIZONTAL, COL_SYM) + BOX_T_UP +
        fillChar(BOX_HORIZONTAL, COL_NEW) + BOX_T_UP +
    ...
    {
      String fileText = "FILE: " + oldPath + " \u2192 " + newPath + " (renamed)";
      int fileLen = display.displayWidth(fileText);
      int padding = width - 4 - fileLen;
     ...
     */
    private String fillChar(char c, int count)
    {
      if (count <= 0)
        return "";
