...
**If pre-rendered banners are not available** (e.g., after context compaction), generate them by running:

```bash
${CLAUDE_PLUGIN_ROOT}/scripts/get-progress-banner.sh "${ISSUE_ID}" --phase <phase_name>
```

Where `<phase_name>` is one of: `preparing`, `executing`, `reviewing`, `merging`.

Output the script result directly. Do NOT manually construct banners.
