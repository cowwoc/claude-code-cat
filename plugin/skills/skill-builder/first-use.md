...
is present, the content is ready — just echo it. If the marker is missing, preprocessing did not run; tell the user to run `/cat:feedback` to report this issue.
```

**first-use.md pattern for handler-preprocessed skills:**

The thin wrapper first-use.md MUST follow this exact pattern:
1. Line 1: `Echo the content from "SKILL OUTPUT X" above exactly as it appears, with no changes.`
2. Line 2: `Do NOT add any other text before or after it.`
3. Explanatory guard: explain the content was generated by a preprocessing hook, state it is ready to
   echo if the marker is present, and that there is nothing to display if the marker is missing. Use
   positive framing ("content is ready — just echo it") rather than negative ("FAILED", "STOP").
