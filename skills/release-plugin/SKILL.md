...
```bash
# 1. Verify versions match (on v2.0.4 branch)
jq '.version' package.json           # "2.0.4"
jq '.version' .claude-plugin/plugin.json  # "2.0.4"

# 2. Verify CHANGELOG.md is complete for v2.0.4
# - Current Version table shows 2.0.4
# - Version history has v2.0.4 entry with all changes listed
...
git tag -a v2.0.4 -m "v2.0.4 release"

# 6. Create next version branch
git checkout -b v2.0.5

# 7. Increment versions
jq '.version = "2.0.5"' package.json > package.json.tmp && mv package.json.tmp package.json
jq '.version = "2.0.5"' .claude-plugin/plugin.json > plugin.json.tmp && mv plugin.json.tmp .claude-plugin/plugin.json

# 8. Create migration script for 2.0.5
cat > migrations/2.0.5.sh << 'EOF'
#!/bin/bash
set -euo pipefail
source "${CLAUDE_PLUGIN_ROOT}/migrations/lib/utils.sh"
log_success "Migration to 2.0.5 completed (no structural changes)"
EOF
chmod +x migrations/2.0.5.sh

# 9. Add migration registry entry
jq '.migrations += [{"version": "2.0.5", "script": "2.0.5.sh", "description": "Migration to 2.0.5"}]' \
  migrations/registry.json > migrations/registry.json.tmp && mv migrations/registry.json.tmp migrations/registry.json

# 10. Update CHANGELOG for next version
# - Update version table to 2.0.5
# - Add empty v2.0.5 section to history

# 11. Commit version bump
git add package.json .claude-plugin/plugin.json CHANGELOG.md migrations/
git commit -m "config: bump version to 2.0.5"

# 12. Push everything
git push origin main
git push origin v2.0.4                # push tag
git push origin --delete v2.0.4       # delete remote branch
git push -u origin v2.0.5
```
...
main ──────────────────●── (tag v2.0.4)
                        ...
                        │ checkout -b v2.0.5
                        ▼
v2.0.5 branch ─────────●── (bump version, commit)
```
