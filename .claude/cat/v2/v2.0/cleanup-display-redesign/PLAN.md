...
## Files to Modify
- plugin/hooks/skill_handlers/cleanup_handler.py - Create new handler for pre-computation
- plugin/commands/cleanup.md - Update survey/plan/completion output to use pre-computed displays
...
1. **Create cleanup_handler.py**
   ...
   - Verify: Handler registered and returns pre-computed output for each phase

2. **Update cleanup.md output steps**
   - Files: plugin/commands/cleanup.md
   - Verify: Skill outputs pre-computed boxes without recalculation
