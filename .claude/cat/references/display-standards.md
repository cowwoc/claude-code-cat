...
Use single-line borders with rounded corners. Content flows freely with indentation.
...
| `┬` `┴` | Metric separator connections (optional) |
| `│` | Metric separator (single line only, connected to borders) |

**Standard width**: 70 characters total for small boxes, 101 for full-width boxes.

### Border Alignment {#border-alignment}

**CRITICAL**: Header and footer must have identical total width.

For a box with total width W:
- **Footer**: `╰` + (W-2) dashes + `╯` = W chars
- **Header**: `╭─ ` + title + ` ` + dashes + `╮` = W chars
  - Dashes needed = W - 5 - display_width(title)

The header needs fewer dashes because of the title. Calculate header dashes to match footer width.
...
Note: The `┌─` and `└─` create visual grouping for major versions within the main box.
...
2. Use rounded corners (`╭╮╰╯`) instead of square (`╔╗╚╝`)
3. Remove vertical borders from content areas
4. Indent content with spaces instead of padding
5. Use `┬│┴` only for metric separators on single lines
