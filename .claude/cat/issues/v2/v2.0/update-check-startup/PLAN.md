...
## Technical Approach
Use GitHub API to fetch latest release from `cowwoc/claude-code-cat` repository. The plugin is installed via GitHub, not npm, so the version check must use the GitHub releases API:

```
GET https://api.github.com/repos/cowwoc/claude-code-cat/releases/latest
```
