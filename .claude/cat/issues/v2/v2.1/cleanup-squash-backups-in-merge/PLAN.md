...
## Execution Steps
1. **Update git-squash-quick.sh:** Remove lines 19-26 (cleanup_backup function + trap), remove lines 151-153 (explicit
   backup deletion on success), remove `BACKUP=""` after deletion. Keep backup creation at line 97-98.
2. **Update merge-and-cleanup.sh:** Add STEP 5 (renumber existing step 5 to 6) that iterates
   `backup-before-squash-*` branches, checks `git merge-base --is-ancestor $backup HEAD`, deletes if true
3. **Test:** Verify merge succeeds and backups are cleaned up
