...
## Goal
Add a license header referencing the CAT Commercial License to all source files in the project, and
...
| XML (*.xml) | N/A | **Skip** - Configuration files |
...
The copyright year is the year the source code was first written. Do not update it over time. When code
is moved or renamed, the copyright year remains the year the source code was originally written.
...
**IMPORTANT:** No blank line should appear between the license header and the first line of code (package declaration, imports, etc.).
```
...
- [ ] JSON and XML files are not modified (JSON has no comment syntax; XML files are configuration files)
- [ ] No blank line appears between license header and first line of code
