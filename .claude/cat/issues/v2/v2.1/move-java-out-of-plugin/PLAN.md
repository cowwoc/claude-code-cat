...
## Current State
Java hooks source lives at `plugin/hooks/java/`. The entire Java project (source, tests, target/) gets copied into the
...
### Move Java source
- `plugin/hooks/java/` → `hooks/` (git mv the entire directory)
...
These files contain `plugin/hooks/java/` paths. Update to `hooks/`:
